    ...
    CD.prototype = {
        ...
        seed: function () {
            ...
            if ($self.defs.isMobile) {
                ...
                if (mobile == '') {
                    $self.layer.msg("请输入手机号码", {icon: 2});
                    return;
                }
                if (!$self.checkMobile(mobile)) {
                    $self.layer.msg("手机号码不正确", {icon: 2});
                    return;
                }
                else {
            ...
            $.ajax({
                    ...
                    if (data.success) {
                        ...
                        if ($self.defs.isCallBack) {
                            if (data[$self.defs.isCallBack]) {
                                $self.layer.msg(data[$self.defs.isCallBack], {icon: 5});
                            }
                        }
                        else {
                            if (data.code == '500') {
                                $self.layer.msg("发送失败,请稍后再试。", {icon: 5});
                            }
                            else {
                                $self.layer.msg("页面已过期，请重新登录并再次发起操作", {icon: 5});
                            }
                        }
                        if ($self.defs.isMsg) {
                            if (data.code != '500') {
                ...
                error: function () {
                    $self.layer.msg("验证码发送失败,请稍后再试。", {icon: 5});
                }
            });
        },
